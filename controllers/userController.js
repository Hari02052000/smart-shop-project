...
module.exports = {
  ...
  signUp: async (req, res) => {
    ...
    try {
      const { username, password, confPassword, email, phone } = req.query
  ...
  incrimentQuantity: async (req, res) => {
    try {
      ...
      let user = req.user
      let userid = user._id
      const cartProduct = user.cart.find(item => item.product.equals(product));

      if (cartProduct.quantity <= 1 && action=='sub') {
        return res.status(400).json({ err:'less than 1' });
      }
      
      if (cartProduct.quantity >= 10 && action=='add') {
        return res.status(400).json({ err:'greater than 10' });
      }
